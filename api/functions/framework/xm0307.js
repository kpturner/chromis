/**           
 *   @Module        xm0307                                                      
 *   @Description   Function Group Maintenance Grid                    
 *   @Author        Gavin Owen                                             
 *   @Date          June 2016                                                      
 */
    ...
    process: function (cb) {
        ...
        switch (self.action) {
            ...
            case "initfuncgroups":
                ...
                funcgrpquery = xfuncgrp.find({ where: Filters.convertToCriteria(filters, "xfuncgrp") })
                    .sort(self.MOD.get("sortby") + " " + self.MOD.get("sortorder").toUpperCase())
                    .skip(startRow - 1)
                    .limit(rows);

                funcgrpquery.exec(function (err, rawData) {

                    // Send our array of func objects.
                    self.MOD.set("data", rawData);
            ...
            case "export":
                Database.exportTable("xfuncgrp", self.MOD.get("selectedfuncgrp"), function (fileName) {
