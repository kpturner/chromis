/**           
 *   @Module        xm9999                                                            
 *   @Description   Derive function application library           
 *   @Author        Kevin Turner                                            
 *   @Date          June 2016                                                      
 */
...
module.exports =  {
    ...
    process: function(cb){
        ...
        try {
            switch (self.action) {
                case "init":
                    var lFunc=self.MOD.get("func");
                    // Search for the file within the functions list
                    self.Func.getApp(lFunc+".js",function(err,applib){
                        if (err) {
                            sails.log.error(err);
                            applib="*error";
                        }
                        self.MOD.set("applib",applib);
                        // Run the callback
                        cb();   
                    })
