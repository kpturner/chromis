        ...
        switch (self.action) {
            case "init":
                ...
                async.parallel([
                    ...
                    function (callback) {
                        var findCriteria = {where:{xwpuser:self.SES.user.xususer,xwpwidg:"presentationrepository"}};
                        var createObject = {xwpuser:self.SES.user.xususer,xwpwidg:"presentationrepository",xwpjson:JSON.stringify({})};
                        xwprefs.findOrCreate(findCriteria,createObject).exec(function(err,createdOrFound){
                            if (err) {
                                sails.log.error(err);
                            }
                            createdOrFound.xwpjson = JSON.parse(createdOrFound.xwpjson);
                            // Send our array of group objects.
                            self.MOD.set("prprefs",createdOrFound);         
                            callback();                                           
                        });      
                    }],
            ...
            case "initpr":
                ...
                var prprefs = self.MOD.get("prprefs");
                
                var prQuery = {};                                            
                var prEntries=[];   
                var criteria = { where : {or:[]}};

                var prtextFilter = Filters.convertToCriteria(filters,"xpr");

                if (!_.isEmpty(prtextFilter)){
                    criteria.where.or.push(prtextFilter);

                    if (prprefs.xwpjson.agnosearch){
                        xtrans.find(criteria).exec(function(err,transData){
                            var prids = [];

                            _.forEach(transData,function(record,i){
                                prids.push(record.pr);
                            });

                            criteria.where.or.push({id:prids});
                ...
                function getPr(){
                    if (criteria.where.or.length == 0){
                        criteria.where = {};
                    }

                    prQuery = xpr.find(criteria)
                                    .sort(self.MOD.get("sortby") + " " + self.MOD.get("sortorder").toUpperCase())
                                    .skip(startRow - 1)
                                    .limit(rows);          
                    prQuery.exec(function(err,rawData){
                        ...
                        _.forEach(rawData,function(record,i){
                            ...
                            r.text   = record.text || "";
                            r.id     = record.id;
                            r.values = (record.inputType === "C" || record.inputType === "D");
                            
                            prEntries.push(r);
                        });
                        
                        // Send our array of role objects.
                        self.MOD.set("prData",prEntries);    
            ...
            case "export":
                var fileName = "prExport" + self.SES.user.xususer;
                Database.exportPr(fileName,self.MOD.get("selectedpr"),function(fileLoc){
                    self.MOD.set("downloaduri",fileLoc);
                    setTimeout(function(){
            ...
            case "saveset":
                ...
                xwprefs.update({where:{id:prprefs.id}},{xwpjson:JSON.stringify(prprefs.xwpjson)}).exec(function(err,updated){
                    cb();
                });
